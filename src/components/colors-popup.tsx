...
const PopoverDemo = ({ type }: { type: string }) => {
  ...
  useEffect(() => {
    document.body.setAttribute(
      'style',
      `--firstColor: ${preview.firstColor}${preview.firstColorOpacity} ; --secondColor: ${preview.secondColor}${preview.secondColorOpacity};`
    );
  });
    ...
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        ...
        >
          ...
          />
          {preview[type as keyof PreviewState]}
        </button>
      </Popover.Trigger>
          ...
          <Popover.Portal forceMount>
            <animated.div style={styles}>
              <Popover.Content
                ...
                forceMount
              >
                <ColorPalette dispatch={dispatch} type={type} preview={preview} />
              </Popover.Content>
            </animated.div>
          </Popover.Portal>
        ) : null
...
const ColorPalette = ({ dispatch, type, preview }: {
  dispatch: Dispatch;
  type: string;
  ...
  const returnHextOpacity = (value: number) => {
    ...
    dispatch({
      type:
        type === 'firstColor'
          ? setFirstColorOpacity.type
          : setSecondColorOpacity.type,
      payload: hex,
    });
  };
  ...
  const colorOpacity =
    type === 'firstColor'
      ? preview.firstColorOpacity
      : preview.secondColorOpacity;
  return (
    <div id="customcolor">
      {colors.map((color) => (
        <button
